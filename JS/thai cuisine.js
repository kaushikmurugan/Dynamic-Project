...
const thaiCuisineData = foodItem.filter((items) => items.cuisine == "TC");
console.log(thaiCuisineData);

let topic = document.createElement("h1");
topic.classList.add("text-center", "text-uppercase");
let tcontent = document.createTextNode("thai cuisine");
topic.appendChild(tcontent);
document.getElementById("thaiitem").appendChild(topic);

let god = document.createElement("div");
god.classList.add("d-flex", "flex-wrap", "justify-content-center");
document.getElementById("thaiitem").appendChild(god);

thaiCuisineData.map((item) => {
